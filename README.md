...
- **青蛙负责原地蹦跶**：8 个像素姿态，从下蹲蓄力、腾空收腿到落地缓冲，
  蹲坐时停留三拍，每 1.2 秒跳一轮。
...
- **只使用系统菜单栏状态项**：不创建普通窗口或悬浮覆盖层，所有操作都从菜单栏
  角色菜单完成；多屏幕、全屏、Space 与自动隐藏状态下，角色由 macOS 菜单栏
  统一显示或隐藏。
- **安静守规矩**：没有监控、分析或网络请求，也不会偷偷向谁汇报你今天看了
  它们多少次。

第一次启动时，TinyTroupe 会先派出一个土下座角色。打开任意角色的菜单，
可以继续添加土下座角色、像素猫或青蛙。选择「青蛙跳」可切换当前角色，
选择「添加动画 → 添加青蛙跳」可再添一只青蛙。每个角色都会记住自己的动画、
朝向和暂停状态；下次打开应用时，整支队伍会回到上次的位置。
...
它完全一致。例如应用版本为 `2.0.0`，标签就必须是 `v2.0.0`：
